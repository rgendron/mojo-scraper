  ...
  class MainPage
    ...
    def parse_players
      if !players_div
        ...
        @directors = dlink ? dlink.parent.parent.next_sibling.css('a').map(&:text) : []

        wlink = players_div.at_css('table>tr>td a[href*=Wri]')
        @writers = wlink ? wlink.parent.parent.next_sibling.css('a').map(&:text) : []

        alink = players_div.at_css('table>tr>td a[href*=Act]')
        @actors = alink ? alink.parent.parent.next_sibling.css('a').map(&:text) : []

        plink = players_div.at_css('table>tr>td a[href*=Pro]')
        @producers = plink ? plink.parent.parent.next_sibling.css('a').map(&:text) : []
